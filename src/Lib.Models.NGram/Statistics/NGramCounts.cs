...
public sealed class NGramCounts
{
    ...
    public int GetBigramCount(int prev, int next) => _bigramCounts[prev, next];
    ...
    public bool HasTrigram(int p2, int p1) => _trigramCounts.ContainsKey((p2, p1));

    public int[] GetTrigramNextCounts(int p2, int p1) =>
        _trigramCounts.TryGetValue((p2, p1), out var arr) ? arr : Array.Empty<int>();
